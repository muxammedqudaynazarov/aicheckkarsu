...
class CheckLessonFiles2 implements ShouldQueue
{
    ...
    private function getAvailableApiAccount($requiredRpd = 1)
    {
        ...
        $account = Account::where('status', '0')->where('rpd', '>=', $requiredRpd)->orderBy('id')->first();

        // 2. Agar aynan shuncha limitli akkaunt topilmasa, borini (RPD > 0) olamiz
        if (!$account) {
            $account = Account::where('status', '0')->where('rpd', '>', 0)->orderBy('rpd', 'desc')->first();
        }
