...
@section('title', 'Mutaxassisliklar ro\'yxati')
                        ...
                        <div class="col-md-6 mb-2">
                            <div class="input-group">
                                <input type="search" name="search_name" id="search_name" class="form-control rounded-0"
                                       placeholder="Mutaxassislik bo‘yicha qidirish" value="{{ request('search_name') }}"
                                       autocomplete="off">
            ...
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title mt-1 font-weight-bold">
                        Mutaxassisliklar ro‘yxati va statistikasi
                    </h3>
                </div>
                ...
                <div class="card-body table-responsive p-0">
                    <table class="table table-hover text-center align-middle small">
                        ...
                        <tr>
                            <th style="width: 5%">#</th>
                            <th style="text-align: start;">Mutaxassislik nomi</th>
                        ...
                        @forelse($specialties as $specialty)
                            <tr>
                                <td class="align-middle">#{{ $specialty->id }}</td>
                                ...
                                <td class="align-middle">
                                    {{ $specialty->department->name ?? 'Biriktirilmagan' }}
                                </td>
                                <td class="align-middle">
                                    {{ $specialty->groups->count() ?? 0 }}
                                </td>
                                <td class="align-middle">
                                    {{ $specialty->groups->sum(fn($group) => $group->students->count()) }}
                                </td>
                                <td class="align-middle">
                                    {{ $specialty->groups->sum(fn($group) => $group->lessons->count()) }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                @if($specialties->total() > 15)
                    <div class="card-footer clearfix bg-white border-top">
                        <ul class="pagination pagination-sm m-0 float-right" id="pagination-container">
                            {{ $specialties->appends(request()->query())->links('pagination::bootstrap-4') }}
                        </ul>
                    </div>
                @endif
            </div>
